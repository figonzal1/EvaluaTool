...
package cl.figonzal.evaluatool.evalua4.modulo1;
...
public class MemoriaAtencionE4M1 extends AppCompatActivity implements EvaluaInterface {
    ...
    public double corregirPD(Integer[][] perc, double pd_total) {
        if (pd_total < 0) {
            ...
            for (Integer[] item : perc) {
                if (pd_total <= item[0] && pd_total >= item[1]) {
                    return pd_total;
                }
            }
        }
